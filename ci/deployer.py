...
from ci.path import CI_ROOT, JENKINS_ROOT, STOCKS_ROOT
...
class Deployer(CI):
    ...
    def add_args(self, argparser: argparse.ArgumentParser):
        ...
        deployer_group.add_argument(
            "--deployer", help="Deploy to k8s", action="store_true"
        )
        deployer_group.add_argument(
            "--deploy-jenkins", help="Deploy to k8s", action="store_true"
        )
        deployer_group.add_argument(
            "--template", help="Output templates", action="store_true"
        )
        deployer_group.add_argument(
            "--ci-mode", help="Set CI Mode", action="store_true"
        )
    ...
    def helm_cmd(
        self, cmd: list[str], name: str, chart: str, values: dict, namespace: str
    ):
        if not self.args.ci_mode:
            cmd = (
                ["helm", "--kube-context", self.context]
                + cmd
                + ["--namespace", namespace, name, chart]
            )
        else:
    ...
    def update_config(self, values: dict):
        ...
        with open(f'/tmp/ci-config.{time.strftime("%Y%m%d-%H%M%S")}.yaml', "w") as f:
            f.write(old.output)
        old = run(
            ["kubectl", "delete", "secret", "--namespace", "jenkins", "ci-config"]
        )
        run(
            [
                "kubectl",
    ...
    def main(self):
        if self.args.deploy_jenkins:
            cmd = [
                ...
                "--install",
                "-f",
                f"{JENKINS_ROOT}/jenkins/values.yaml",
                "-f",
                f"{JENKINS_ROOT}/values.yaml",
            ]
            cmd = self.helm_cmd(
                cmd, name="jenkins", chart=".", values={}, namespace="jenkins"
            )
        ...
        if self.args.deployer:
            ...
            if self.args.ci_mode:
                ...
                values = {
                    ...
                    "grafana.datasources[0].secureJsonData.token": CONFIG["grafana"][
                        "influxdb"
                    ]["token"],
            ...
            cmd = ["upgrade", "--install"]
            cmd = self.helm_cmd(
                cmd, name="stock-trading", chart=".", values={}, namespace="stock"
            )
            if self.args.template:
                cmd = ["template", "--debug"]
                cmd = self.helm_cmd(
